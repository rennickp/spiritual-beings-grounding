...
error SpiritualBeingsGrounding__CurrentlyGrounding();
error SpiritualBeingsGrounding__NotApprovedOrOwner();
error SpiritualBeingsGrounding__GroundingClosed();
error SpiritualBeingsGrounding__NotGrounded();

contract SpiritualBeingsGrounding is ERC721, Ownable {
  /////////////////////
  // State Variables //
  /////////////////////
  ...
  modifier onlyApprovedOrOwner(uint256 tokenId) {
    if (!_isApprovedOrOwner(msg.sender, tokenId)) {
      revert SpiritualBeingsGrounding__NotApprovedOrOwner();
    }
    _;
  }
  ...
  function _beforeTokenTransfer(
    ...
    if (!(s_groundingStarted[tokenId] == 0 || s_groundingTransfer == 2)) {
      revert SpiritualBeingsGrounding__CurrentlyGrounding();
    }
  }
    ...
    onlyApprovedOrOwner(tokenId)
  {
    ...
    if (start == 0) {
      if (!s_groundingOpen) {
        revert SpiritualBeingsGrounding__GroundingClosed();
      }
  ...
  function expelFromGrounding(uint256 tokenId) external onlyOwner {
    if (s_groundingStarted[tokenId] == 0) {
      revert SpiritualBeingsGrounding__NotGrounded();
    }
